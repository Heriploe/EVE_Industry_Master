...
# find_repo_root 已由 load_app_config 替代


def resolve_path(repo_root: Path, fallback: str) -> Path:
    path = Path(fallback)
...
def main() -> None:
    ...
    default_region_ids = parse_region_ids(cfg.get("fetch", {}).get("region_ids", [10000002])) or DEFAULT_REGION_IDS
    default_request_interval = float(cfg.get("fetch", {}).get("request_interval", DEFAULT_REQUEST_INTERVAL))
    ...
    alias_file = resolve(eve_root, cfg.get("data", {}).get("materials_alias_json", "Data/Materials/alias.json"))
    preset_file = resolve(eve_root, cfg.get("data", {}).get("materials_preset_json", "Data/Materials/preset.json"))
    cache_dir = resolve(eve_root, cfg.get("data", {}).get("market_cache_dir", "Cache/Market"))
