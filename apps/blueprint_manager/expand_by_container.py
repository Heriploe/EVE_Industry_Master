...
from utilities.blueprint.blueprint_utils import load_blueprints_from_file as load_blueprints
...
def main():
    ...
    default_input = None  # 由 cfg["resources"]["corp_blueprints"] 决定
    default_inventory = None  # 由 cfg["resources"]["corp_materials"] 决定
    default_output = None  # 由 cfg["output_dir"] 决定
    default_depth = int(_exp.get("max_depth", 0))
    default_root_runs = float(_exp.get("root_runs", 1.0))
    ...
    types_json = resolve_path(eve_root, cfg["data"]["types"])
    blueprints_yaml = resolve_path(eve_root, cfg["data"]["blueprints_yaml"])
