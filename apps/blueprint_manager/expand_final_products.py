...
def main() -> None:
    ...
    _exp = cfg.get("expand", {})
    default_final = str(resolve_path(cfg.get("output_dir", "outputs/blueprint_manager"), eve_root) / "final_products.csv")

    default_depth          = int(_exp.get("max_depth", 0))
    default_decryptor_name = _exp.get("decryptor_name", "").strip()
    ...
    types_json      = resolve_path(cfg["data"]["types"], eve_root)
    blueprints_yaml = resolve_path(cfg["data"]["blueprints_yaml"], eve_root)
    t2_t1_path      = resolve_path(cfg["data"]["t2_map"], eve_root)
