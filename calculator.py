...
def _load_product_ids_from_blueprint_preset(alias_file: Path, preset_file: Path, preset_name: str):
    _, blueprints = _load_blueprints_by_preset(alias_file, preset_file, preset_name)
    product_ids = set()
...
INTERMEDIATE_MATERIALS_PRESET = config.get("calculator", "intermediate_materials_preset", fallback="intermediate_materials")
...
INTERMEDIATE_PROFIT_FACTOR = config.getfloat("calculator", "intermediate_profit_factor", fallback=1.0)
...
# ------------------ 读取各类 preset ------------------
ship_ids = _load_product_ids_from_blueprint_preset(BLUEPRINTS_ALIAS_JSON, BLUEPRINTS_PRESET_JSON, SHIPS_PRESET)
module_product_ids = _load_product_ids_from_blueprint_preset(BLUEPRINTS_ALIAS_JSON, BLUEPRINTS_PRESET_JSON, MODULES_PRESET)
rig_product_ids = _load_product_ids_from_blueprint_preset(BLUEPRINTS_ALIAS_JSON, BLUEPRINTS_PRESET_JSON, RIGS_PRESET)
intermediate_material_ids = _load_product_ids_from_blueprint_preset(
    BLUEPRINTS_ALIAS_JSON, BLUEPRINTS_PRESET_JSON, INTERMEDIATE_MATERIALS_PRESET
)
...
def get_product_profit_factor(product_type_id):
    ...
    if tid in intermediate_material_ids:
        return INTERMEDIATE_PROFIT_FACTOR
    if tid in ship_ids:
