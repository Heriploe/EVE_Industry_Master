...
from market_order_utils import fetch_best_buy, find_type_id
...
def main():
    ...
    threshold     = config.getfloat("sell_tools","scrap_threshold",      fallback=0.80)
    out_file      = config.get("sell_tools","scrap_outfile",             fallback="items_to_scrap.csv")
    mat_file      = str(_REPO / config.get("sell_tools","scrap_materials_file",
                                           fallback="Data/typeMaterials.yaml"))
    cache_file    = str(_REPO / config.get("sell_tools","scrap_cache_file",
                                           fallback="Cache/Market/scrap_price_cache.json"))
    refresh_cache = config.getboolean("sell_tools","scrap_refresh_cache", fallback=False)
