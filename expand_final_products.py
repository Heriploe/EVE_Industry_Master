...
def main():
    ...
    parser.add_argument("--max-depth", type=int, default=default_depth, help="展开深度，默认 0")
    parser.add_argument("--decryptor-name", default=default_decryptor_name, help="解码器中文名（来自 config，可覆盖）")
    ...
    def expand_product(type_id: int, qty: float, depth: int, stack: Set[Tuple[int, str]]):
        ...
        if bp_id in t2_to_t1:
            ...
            if decryptor_id is not None:
                add_missing_material(int(decryptor_id), required_invention_runs)
