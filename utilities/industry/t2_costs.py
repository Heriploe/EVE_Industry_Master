...
from utilities.industry.cost import (
    get_T1_from_T2,
    invention_T2_runs,
    _load_blueprints,
...
try:
    _app_cfg, REPO_ROOT = load_app_config()
    _meta = load_meta(REPO_ROOT)
except Exception:
...
def _build_costs():
    cfg = _load_config()
    jita_path            = _resolve(_resolve("resources/market/price_all.json"))
    blueprints_yaml_path = _resolve(_resolve("Data/blueprints.yaml"))
    t2_t1_path           = _resolve(_resolve("Data/T2_T1.json"))
    decryptor_path       = REPO_ROOT / "Data/Materials/Basic/decryptor.json"
    price_adjusted_path  = _resolve(_resolve("Data/price_adjusted.json"))
    types_json_path      = _resolve(_resolve("Data/types.json"))
