...
def fetch_min_sell(
    ...
    while True:
        ...
        if not orders:
            break
        for o in orders:
            is_buy = o.get("is_buy_order", False)   # ESI 始终提供此字段，False 为安全默认值
            if order_type == "sell" and is_buy:
                continue
            if order_type == "buy" and not is_buy:
                continue
            if station_id and o.get("location_id") != station_id:
                continue
            p = o.get("price")
...
def fetch_structure_prices(
    ...
    for o in all_orders:
        if o.get("is_buy_order", False):   # False 为安全默认值：缺失时当作 sell 单保留
            continue
...
def get_structure_token(meta_or_cfg, repo_root: Path) -> Optional[str]:
    """
    获取有效 Bearer token。
    meta_or_cfg: config_meta.json 内容（dict），或旧式 configparser 对象（兼容）。
    """
    try:
        from utilities.esi.esi_auth import get_valid_token as _gvt
    except ImportError:
        try:
            from esi_auth import get_valid_token as _gvt
        except ImportError:
            print("[market_order_utils] ✗ 无法导入 esi_auth")
            return None

    # 支持 dict（config_meta）和旧式 configparser
    if isinstance(meta_or_cfg, dict):
        esi = meta_or_cfg.get("esi", {})
        cache_raw = meta_or_cfg.get("token_cache", "resources/auth/token_cache.json")
        cache_path = Path(cache_raw) if Path(cache_raw).is_absolute() else repo_root / cache_raw
        settings = {
        ...
        # 兼容旧式 configparser
        cache_raw  = meta_or_cfg.get("esi_auth","token_cache_file",fallback="resources/auth/token_cache.json")
        cache_path = Path(cache_raw) if Path(cache_raw).is_absolute() else repo_root / cache_raw
        settings = {
            "client_id":     meta_or_cfg.get("esi_auth","client_id",     fallback=""),
            "client_secret": meta_or_cfg.get("esi_auth","client_secret", fallback=""),
            "redirect_uri":  meta_or_cfg.get("esi_auth","redirect_uri",  fallback=""),
            "scope":         meta_or_cfg.get("esi_auth","scope",         fallback=""),
            "cache_file":    str(cache_path),
            "user_agent":    meta_or_cfg.get("esi_auth","user_agent",    fallback="EVE/1.0"),
        }
    try:
